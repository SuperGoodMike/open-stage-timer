...
function hms(s){s=Math.max(0,Math.floor(Number(s)||0));const h=String(Math.floor(s/3600)).padStart(2,"0"),m=String(Math.floor((s%3600)/60)).padStart(2,"0"),sec=String(s%60).padStart(2,"0");return `${h}:${m}:${sec}`;}

export default function TimerPanel({ timer, mode, setMode, inputTime, setInputTime, beepEnabled, setBeepEnabled }) {
  ...
  const nudge = (delta) => { const x = Math.max(0, (Number(inputTime)||0) + delta); setInputTime(x); socket.emit("set_timer", x); };
  const changeMode = (m) => { setMode(m); socket.emit("set_mode", m); };
  ...
  return (
    <div className="timer-panel">
      ...
      <div className="timer-big">{hms(timer.time)}</div>
      ...
      <div className="timer-controls">
        ...
        {mode !== "clock" && (
          <>
            <div className="row">
              <label>Seconds</label>
              <input type="number" min={0} value={inputTime} onChange={(e)=>setInputTime(Number(e.target.value))}/>
